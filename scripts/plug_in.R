...
for(i in 1:Ny){
  ...
  for(j in 1:dim(Lx)[1]){
    E[,,i]=E[,,i]+as.double(Lx[j,])%*%t(as.double(Lx[j,]))
  }
  E[,,i]<-E[,,i]/(dim(Lx)[1]-1)
}
#Функция вычисляющая плотность распределения величины
Ng=function(x,myu,E){
  n=length(x)
  k=as.double(-1*((t(x-myu)%*%solve(E))%*%(x-myu))/2)
...
e<-0.0
for(i in 1:l){
  e<-e+100*(Xl[,nomer_stolbtca_classov][i]!=a(c(Xl[,1][i],Xl[,2][i]), classes, Py, myus, E ))/l
}
...
while(w<w_max){
  h=h_min
  while(h<h_max){
    points(w, h, col = tcveta[a(c(w,h), classes, Py, myus, E )], pch = 1)
    h=h+0.1
  }
  w=w+0.1
}
