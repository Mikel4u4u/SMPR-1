...
P<sub>y</sub> = P(y) называются априорными вероятностями классов.</p>
 ...
  <p><i>Функционалом среднего риска</i> называется ожидаемая величина потери при классификации объектов алгоритмом a:</p>
  ...
  <p><i>Разделяющая поверхность</i> между классами s и tэто геометрическое место точек x ∈ X таких, что минимум среднего риска достигается одновременно при y = s и y = t:</p>
<p>λ<sub>t</sub>P<sub>t</sub>p<sub>t</sub>(x) = λ<sub>s</sub>P<sub>s</sub>p<sub>s</sub>(x). Объекты x, удовлетворяющие этому уравнению, можно относить к любому из двух классов s, t, что не повлияет на средний риск R(a).</p>
  <p>Апостериорная вероятность класса y для объекта xэто условная вероятность P(y|x). Она может быть вычислена по формуле Байеса, если известны py(x) и Py:</p>
